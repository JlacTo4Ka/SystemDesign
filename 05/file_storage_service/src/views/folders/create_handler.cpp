...
std::string CreateHandler::HandleRequestThrow(
  ...
  const auto folder = m_storage.CreateFolder(
      request_dto.owner_user_id,
      request_dto.name);

  redis_client_->Del("cache:folders:pg:all", redis_cc_).Get();

  const auto response_dto = dto::folders::ToFolderResponseDto(folder);

  response.SetStatus(userver::server::http::HttpStatus::kCreated);
  return userver::formats::json::ToString(
      dto::folders::FolderResponseToJson(response_dto));
}
